...
- 다음 작업: **1.1 하네스 네이티브 기능 조사 및 설계 검증**
- 마지막 갱신: 2026-09-20
...
- [ ] **역할의 네이티브 단위** — Claude는 subagent(`.claude/agents/`)와
      skill(`.claude/skills/`) 중 무엇이 역할에 맞는가. Codex는 custom agent와 skill 중 무엇인가
- [ ] **도구 제한 구문** — 역할별 도구 허용/차단을 네이티브로 거는 방법.
      1.5의 매핑 테이블이 이것과 중복인지 판정
- [ ] **스킬 바인딩 구문** — 특정 역할에 특정 스킬만 노출하는 네이티브 방법이 있는가 (1.5 입력)
- [ ] **위임 방식** — 역할 간 호출을 네이티브로 어떻게 하는가. D2 디스패처가 이를 대체하는가 보완하는가
- [ ] **병렬 실행** — 동시 실행을 네이티브로 지원하는가. D3의 전제 확인
- [ ] **컨텍스트 전달** — 서브에이전트에 컨텍스트를 넘기는 네이티브 경로.
      D5 핸드오프가 중복인지 판정
...
- [ ] `docs/development/HARNESS_CAPABILITIES.md` — 하네스별 기능 대조표.
      각 항목에 확인한 공식 문서 출처를 남긴다
- [ ] D2~D6 각각에 **네이티브 / 보완 / 충돌** 판정 기록
- [ ] 충돌 판정이 나온 결정은 `DESIGN_DECISIONS.md`를 먼저 수정하고 로드맵을 조정

완료 기준:
- 대조표의 모든 항목에 출처가 있다. 추측으로 채운 칸이 없다
- Claude 어댑터 `fileName` 문제에 결론이 났다
- 설계 결정 중 "충돌" 판정이 하나라도 있으면 해당 D 항목이 갱신되었다
...
- [ ] manifest 역할에 `skills` 키를 추가한다 (역할이 호출 가능한 재사용 절차)
...
{
  "name": "role-analyst",
  "skills": ["web-research", "metric-query"],
