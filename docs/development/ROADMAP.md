...
- 진행 단계: **Phase 1 머지 완료** (`main` = PR #4 → #7 → #5). Phase 2 미착수
- 다음 작업: **Phase 2 대상 선정** — 사용자 결정이 필요하며 그 전에는 착수할 수 없다
- 마지막 갱신: 2026-09-26
...
| 도구 | 명령 | 비용 |
|---|---|---|
| 구조·렌더링·패키지 검사 | `npm run check` | 무료, 매번 |
| 동작 검증 (Claude) | `npm run eval` | 유료, 수동 |
| 동작 검증 (Codex) | `AGENT_WORKFLOW_EVAL_BASE=<신뢰된 경로> node tests/eval/run.js --target codex` | 유료, 수동 |

역할 10개가 배포되고 두 하네스 모두 검증 경로가 있다.
- 마지막 갱신: 2026-09-20
...
착수 전에 넷을 모두 확인한다. 하나라도 어긋나면 대상을 바꾼다.

| # | 기준 | 어긋나면 |
|---|---|---|
| 1 | **테스트 가능한 로직이 있다** | `role-qa`가 할 일이 없어 게이트를 만들 수 없다. 순수 CRUD 화면만 있는 서비스가 여기 걸린다 |
| 2 | 사용자가 도메인을 잘 안다 | 산출물이 틀린 것을 알아볼 수 없어 측정이 무의미하다 |
| 3 | 사람 혼자 2~3일 크기다 | "잘 됐다"를 말할 비교 기준이 없다 |
| 4 | 배포 가능하다 | Phase 3에서 `role-analyst`의 관측 대상이 되지 못한다 |

1번이 가장 중요하다. 나머지 셋은 절충할 수 있지만 1번이 없으면 Phase 2의 목적
자체가 사라진다. 자율성의 상한은 검증 가능성이 결정한다 (D7).
