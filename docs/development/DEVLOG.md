...
## 2026-09-26 (23) — Phase 1 머지 (PR #4 → #7 → #5)

- **머지 완료.** `main` = `8440ee5`
  - #4 Phase 1 (1.1~1.9) + 배포 레이어 분리 + CHANGELOG — 18커밋
  - #7 Codex 역할 로딩 복구 + 하네스 Codex 확장 — 2커밋
  - #5 핸드오프 봉투 계약 결함 셋 (R4) — 1커밋

- **머지 중 실수 하나**
  - #4를 `--delete-branch`로 머지했더니 base 브랜치가 사라져 **스택 PR #6이 자동
    CLOSED** 됐다. base 브랜치가 없으면 재오픈도 base 변경도 불가하다
  - 커밋과 브랜치는 남아 있어 유실은 없었다. 같은 내용으로 #7을 열어 머지했다
  - **스택 PR을 머지할 때는 하위 PR이 남아 있는 동안 base 브랜치를 삭제하지 않는다.**
    #7은 `--delete-branch` 없이 머지했다

- **머지된 상태를 tarball로 검증했다**
  - 9개 에이전트 + 1개 스킬 (claude), 9개 (codex)
  - `web_search`가 열거값 문자열이다. boolean 결함이 실제로 고쳐졌다
  - 봉투 플레이스홀더가 `<파일 경로>:<줄 번호>` 형태다
  - 레이어 1 파일 유출 없음

- **Phase 2 착수 전 남은 결정**
  - **미니 프로젝트 대상.** 사용자 결정이 필요하며 그 전에는 착수할 수 없다
  - 선정 기준 4개를 ROADMAP에 표로 정리했다. 1번(테스트 가능한 로직)이 가장 중요하다.
    나머지는 절충할 수 있지만 1번이 없으면 Phase 2의 목적 자체가 사라진다
  - 측정 항목도 착수 전에 확정한다. 특히 "개입 이유 분류"가 Phase 3의 자동화 범위를
    그대로 결정한다

- **이월된 항목**
  - Codex R4 잔차 2건 (`needs: []`, `from: role-orchestrator`) — Codex 표면화 특성
  - Codex 라우팅은 부모 스트림에서 관찰 불가
  - `mutationPolicy: none` 역할의 동시 배정 케이스 (researcher + analyst)
  - 릴리스 전 버전 범프. 경로 변경이 있으므로 최소 minor

- **다음**: Phase 2 대상 선정 (사용자 결정)

---

## 2026-09-24 (22) — R4 분류와 수정

- **분류 결과: 원인 셋, 전부 계약 결함**
  1. **봉투 예시를 그대로 복사했다.** `path/to/file.ts:42`는 내 템플릿의 플레이스홀더다.
     1.6의 `Source: skills/role-*.md`와 **같은 실수를 두 번째로 반복했다.**
     진짜처럼 보이는 예시는 복사된다
  2. **Stop Conditions가 자기모순이었다.** architect의 "후보만 남긴다"가 Stop Condition
     안에 있어 역할이 스스로를 `complete`로 판단했다
  3. **빈 목록이 허용된다는 규칙이 없었다.** 채울 것이 없으면 예시를 복사했다

- **수정**
  - 플레이스홀더를 `<파일 경로>:<줄 번호>` 형태로
  - 부분 산출물을 만들었어도 `needs`를 채우고 `blocked`로 두도록 명시
  - `facts_confirmed: []`가 정직한 답이라고 명시
  - `validate`가 현실적인 예시 출처를 실패로 처리한다 (같은 실수 세 번째 방지)
  - 판정기에 `envelope.no_template_copy` 추가

- **실패한 시도 하나 — 측정이 아니었으면 개선인 줄 알았을 것이다**
  - 예시를 **빈 봉투**로 바꾸면 복사해도 유효할 것이라 생각했다
  - 결과는 **4/5 → 0/4**. 빈 예시는 복사하기 더 쉬웠고 `needs`까지 비워졌다
  - 측정 결과를 보고 되돌렸다. 영리해 보이는 수정도 측정 없이는 개선인지 알 수 없다

- **진행**: 1/5 → 2/5 → 4/5 → (실패한 시도 0/4) → 되돌린 뒤 3/5
  - `envelope.source_exists`와 예시 복사는 **0건으로 제거됐다**
  - `needs.any_of` 실패는 4/5 → 1/5로 줄었다

- **남은 잔차와 그 성격**
  - `needs: []` (1/5), `from: role-orchestrator` (1/5)
  - 후자는 **Codex가 서브에이전트 봉투를 표면화하는 방식**의 문제다.
    역할 출력을 그대로 반환하게 한 프로브에서 architect는 올바른 봉투를 냈다
    (`status: blocked`, `needs: [code-evidence, product-intent]`, `facts_confirmed: []`)
  - Claude에서는 같은 케이스가 기록된 4회 중 4회 통과한다
  - 따라서 잔차는 계약 결함이 아니라 Codex 표면화 특성으로 본다

- **다음**: Phase 2 대상 선정

---

## 2026-09-24 (21) — Codex 지원 복구와 하네스 확장

- **🔴 1.5부터 Codex 역할이 하나도 로드되지 않고 있었다**
  - `web_search = true`로 렌더링했는데 Codex는 이 키를 열거값으로만 받는다
    (`disabled` / `cached` / `indexed` / `live`)
  - boolean을 쓰면 **역할 파일 전체가 malformed로 무시된다.** 9개 전부였다
  - 설치는 성공하고 `doctor`도 통과한다. 파일이 있는지만 보기 때문이다
  - `web_search = "live" | "disabled"`로 고쳤다. `validate`가 열거값을 검사한다

- **앞선 검증이 이것을 놓친 이유 — 내 문서에 내 검증이 속았다**
  - (20)에서 "9개 에이전트 노출 확인"이라고 기록했는데, 그 답은 도구 정의가 아니라
    `AGENTS.md`에서 나온 것이었다. 1.8에서 안내 블록에 역할 이름을 나열했고
    모델이 그것을 읽어 답했다
  - 다시 확인할 때 "AGENTS.md를 읽지 말라"고 명시하고 `error` 항목이 0개임을
    함께 봤다. **모델에게 묻는 검증은 모델이 어디서 답을 얻었는지까지 통제해야 한다**
  - (20)의 "봉투 미산출"도 같은 결함의 증상이었다. 역할이 실행되지 않았으니
    봉투가 있을 리 없다. TOML을 고치니 R6이 통과한다

- **하네스를 Codex로 확장했다**
  - `--target codex`. `codex exec --json`의 JSONL을 관찰한다
  - **신뢰 가드**: 신뢰되지 않은 경로에서 Codex를 돌리면 역할이 하나도 로드되지 않은
    채 "위반 없음"처럼 보인다. 측정 전에 중단한다
  - `AGENT_WORKFLOW_EVAL_BASE`로 픽스처 기준 경로를 지정한다

- **관찰기에서 같은 실수를 반복했다**
  - Claude 때와 똑같이 항목 JSON에서 역할 이름을 훑었다. 결과도 똑같았다 —
    한 줄 수정 요청에 10개 역할이 잡혔다
  - 이번엔 오케스트레이터 계약을 `cat`하는 `command_execution`과 역할을 언급하는
    `agent_message`가 원인이었다
  - `collab_tool_call`만 위임으로 센다

- **Codex의 관찰 한계를 인정했다**
  - `collab_tool_call`의 `receiver_thread_ids`와 `agents_states`가 비어 있어
    **어떤 에이전트를 띄웠는지 알 수 없다.** Claude는 `subagent_type`으로 알 수 있다
  - 하네스는 Codex에서 위임 횟수만 세고 역할 이름은 판정하지 않는다.
    신호가 없는데 판정하면 거짓 결과가 나온다

- **`setup.roles_loaded` 검사 추가**
  - 역할 정의 로드 실패를 계약 결함으로 판정한다. 이 버그 유형이 다시 조용히
    지나가지 않는다

- **Codex 베이스라인**: 50/51 검사, 6/7 케이스. R2와 R5가 위임 0회로 정확히 통과한다.
  남은 실패는 R4의 `needs.any_of` 1건이며 분류가 필요하다

- **다음**: R4 분류, 그리고 Phase 2 대상 선정

---

## 2026-09-24 (20) — Codex 런타임 검증

- **PR의 마지막 미검증 항목을 닫았다**
  - trust가 상위 경로에서 상속되므로 이미 신뢰된 경로 아래에 검증 프로젝트를 만들었다
  - `doctor`가 `trusted via /Users/hankim/Desktop/workspace` 보고
  - **custom agent 9개 + 오케스트레이터 스킬 전부 노출 확인.**
    trust 가설 확증: 신뢰되지 않으면 `NONE`, 신뢰되면 9개 전부

- **배정도 실제로 동작한다**
  - "기획부터 시작해줘" 요청에 오케스트레이터가 `role-planner`만 활성화하고
    designer/developer를 **명시적 이유와 함께 제외**했다. 과잉 위임 없음
  - `articulate.md`가 실제 내용으로 작성됐고 `app.js`는 건드리지 않았다
  - 4개 출력 블록(`task_classification`, `active_roles`, `role_handoff_blocks`,
    `final_summary`) 정상 산출

- **⚠️ 새로 발견한 공백: Codex 오케스트레이터가 봉투를 산출하지 않는다**
  - 출력에 `from: role-*` 블록이 3개 있었으나 전부 계약 안의 **템플릿 텍스트**였다.
    플레이스홀더가 그대로였고 실제로 채워진 봉투는 **0개**
  - 처음엔 grep 개수만 보고 "봉투 9개 필드 등장"으로 읽었다. 템플릿과 실제 산출물을
    구분하지 않으면 있지도 않은 준수를 확인했다고 착각한다. 플레이스홀더 존재 여부로
    다시 판정해 0개임을 확인했다
  - Claude 경로는 R6이 5/5로 봉투를 산출한다. Codex 고유이거나 오케스트레이터 고유다
  - 1회로는 계약 결함인지 모델 변동인지 구분할 수 없다

- **하네스 커버리지 공백**
  - `tests/eval/run.js`가 `claude` CLI만 구동한다. **Codex 계약 준수는 자동 검증되지
    않는다.** 위 발견도 수동 실행으로 찾았다
  - Phase 1 이후 항목으로 ROADMAP에 남겼다

- **정리**: 검증용 임시 디렉터리를 삭제했다

- **다음**: Phase 2 미니 프로젝트 대상 선정 (사용자 결정 필요)
...
- **작업**
  - `buildRoleGraph` — 간선을 `consumes`로만 만든다. `optionalConsumes`는 없어도
    배정되므로 교착을 만들지 않는다. 선택 입력을 간선으로 넣으면 존재하지 않는
    순환이 잡힌다
  - `findGraphCycles` — DFS 색칠로 필수 간선의 순환을 찾는다
  - `findDispatchOrder` — 환경 입력에서 출발하는 고정점 계산.
    끝까지 들어오지 못한 역할은 어떤 경로로도 배정되지 않는다
  - `validate`가 `[graph] 10 role(s) reachable, no dependency cycles`를 보고한다

- **검증 (D12)**
  - 순환 주입: `role-planner <- role-developer <- role-planner` 잡음
  - 자기 산출물 전제: `role-qa`가 `test_plan`을 소비하게 하니 도달 불가로 잡음
  - 연쇄 도달 불가: releaser가 막히니 그에 의존하는 qa까지 함께 잡음
  - **첫 시도의 테스트가 틀렸다.** `role-qa`가 `release_plan`을 소비하게 했는데
    releaser가 그것을 생산하므로 실제로는 도달 가능한 그래프였다.
    검사가 아니라 내 테스트가 잘못이었고, 진짜 도달 불가 형태로 다시 확인했다

- **Phase 1이 끝났다**

  | 작업 | 핵심 |
  |---|---|
  | 1.0 | 레이어 경계. 루트의 제품 사본 제거 |
  | 1.1 | 하네스 네이티브 조사. D3/D4/D14가 네이티브임을 확인 |
  | 1.2 | 역할 시스템 통합. **Claude 타깃이 로드조차 안 되던 결함 수정** |
  | 1.2b | 레거시 경로 마이그레이션. cursor update 불가 회귀 수정 |
  | 1.3 | Activation / Done / Stop |
  | 1.4 | 핸드오프 봉투 스키마 |
  | 1.5 | 도구 바인딩을 선언에서 도출. **reviewer가 읽기 전용이 아니던 결함 수정** |
  | 1.6 | 동작 검증 하네스. 판정기와 실행기 분리 |
  | 1.7 | 고정 라우팅 표 폐기 → 의존성 해소 |
  | 1.8 | 역할 3개를 선언만으로 추가 (1.7의 실질 검증) |
  | 1.9 | 그래프 순환·도달성 검증 |

  하네스가 없었으면 찾지 못했을 제품 결함이 셋이었다.
  Claude 타깃 미로드, reviewer의 읽기 전용 위반, 배포 파일의 잘못된 `Source:` 경로.

- **Phase 2로 넘어가기 전 필요한 결정**
  - 미니 프로젝트 대상이 아직 미정이다. ROADMAP Phase 2의 선정 기준 4개
    (테스트 가능한 로직 / 도메인 친숙도 / 2~3일 크기 / 배포 가능)를 만족하는지
    확인이 필요하다
  - 측정 항목도 착수 **전에** 확정해야 한다. 특히 "개입 이유 분류"가
    Phase 3의 자동화 범위를 결정한다

- **미해결 (이월)**
  - `mutationPolicy: none` 역할의 동시 배정 케이스. researcher와 analyst가 둘 다
    `none`이라 조합은 생겼지만 이를 강제하는 케이스가 없다
  - `role-qa`의 테스트 파일 제한은 네이티브로 강제되지 않는다 (D15 한계)
...
- **작업**
  - manifest에 `capabilities` 추가 (D16 어휘). `handoffInputs`/`handoffOutputs`를
    `consumes`/`produces`로 **이름만 바꿨다** — 새 키를 더하면 진실 원본이 둘이 된다
  - `environmentInputs`(`user_request`, `workflow_state`) 선언. 생산자가 필요 없는 키다
  - 오케스트레이터의 고정 라우팅 표를 `## Dispatch` 의존성 해소 절차로 교체
  - `## Parallel Dispatch`(D3), `## Dispatch Limits`(D6) 명시
  - `validate`: capabilities 어휘 제한, 소비 키의 생산자 존재, 고정 체인 재유입 차단

- **빠뜨릴 뻔한 것**
  - 오케스트레이터에게 "선언을 읽어 배정하라"고 했는데 **배포 파일에 선언이 없었다.**
    자기 계약 요약만 있고 다른 역할의 `capabilities`/`produces`/`consumes`가 없어서
    의존성 해소가 성립하지 않는 상태였다
  - 렌더러가 각 역할 파일에 자기 선언을, 오케스트레이터 파일에 **전체 역할 명부**를
    주입하도록 고쳤다. 배포물을 직접 열어보지 않았으면 놓쳤을 결함이다

- **의존성 그래프의 구멍 하나**
  - `role-reviewer`가 `specs_doc`을 소비하는데 `role-developer`의 `produces`에
    없었다. `requiredOutputs`에는 있었지만 핸드오프 선언에서 빠져 있었다.
    새 그래프 검사가 바로 잡아냈다
...
- **전후 비교 (1.7의 필수 검증)**
  ...
  | | 이전 (1.6 베이스라인) | 이후 (1.7) |
  |---|---|---|
  | 기록 | `2026-09-23T14-38-25` | `2026-09-24T01-33-34` |
  | 검사 | 41/41 | 41/41 |
  | 케이스 | 6/6 | 6/6 |

  **회귀 없음.** 그리고 역할 조합이 달라졌다. R5가 베이스라인에서는 역할 없이
  직접 처리됐고 1.7에서는 developer가 배정됐다. 둘 다 기대를 만족하며,
  배정이 표가 아니라 계산에서 나온다는 증거다

- **하네스 보강 (중간에 필요해짐)**
  - 첫 1.7 실행이 사용량 한도로 대부분 무효였는데, R2/R3/R5가 "아무것도 안 함"으로
    **거짓 통과**했다. 기대값이 `roles_expected: []`라 0 활동이 통과로 보인 것이다
  - `result` 이벤트의 `is_error`/`subtype`/`api_error_status`로 무효를 판정하게 했다.
    회귀 테스트로 고정했다
  - 거짓 통과는 거짓 실패보다 위험하다. 하네스를 믿게 만들면서 틀린 신호를 준다

- **미해결**
  - `mutationPolicy: none` 역할의 동시 배정은 1.8 이후 검증. 현재 `none` 역할 중
    같은 스텝에 배정 가능한 조합이 나오는 케이스가 없다
  - 제공자 없는 능력 2개(`external-evidence`, `verification`)는 1.8이 채운다.
    `validate`가 `[gap]`으로 보고한다

- **다음**: ROADMAP 1.8 신규 역할 3개 (analyst, qa, releaser)

---

## 2026-09-23 (15) — R6 발견 분류와 해결

- **분류**: `--runs 5`로 R6을 돌려 **2/5 통과**. 실패 형태가 매번 동일했다.
  무작위 변동이 아니라 체계적 패턴이므로 **계약 결함**으로 분류했다 (D12)

- **추적 결과 원인이 다섯이었다**

  | # | 원인 | 분류 | 조치 |
  |---|---|---|---|
  | 1 | "X가 없다"는 사실의 출처 자리가 없어 `source`에 서술을 넣음 | 계약 공백 | `glob:<패턴>` 형태 추가 |
  | 2 | `needs`의 채워진 형태가 계약에 없어 역할이 설명을 덧붙임 | 계약 공백 | 토큰 하나만 쓰도록 명시 |
  | 3 | 아직 만들지 않은 파일을 출처로 인용 | 계약 공백 | 명시적 금지 |
  | 4 | 쓰기 권한 없이 docs-only 역할 실행 | **하네스 환경** | `--permission-mode acceptEdits` |
  | 5 | 배포 파일의 `Source: skills/...` | **제품 렌더링 결함** | `Contract:` 표기로 변경 |

- **4번과 5번이 이 작업의 핵심 수확이다**
  - 하네스가 **자기 환경 제약과 제품 결함을 계약 위반으로 오판**하고 있었다
  - 특히 5번: 배포된 역할 파일이 소비자 프로젝트에 **존재하지 않는 경로**를
    `Source:`라고 알려주고 있었다. 역할이 그것을 근거로 인용한 것은 합리적 행동이다.
    **하네스가 없었으면 찾지 못했을 제품 결함이다**
  - 판정기의 실패를 곧바로 "모델이 이상하다"로 해석하면 안 된다는 사례

- **진행**: 2/5 → (계약 1~3) 4/5 → (환경 4) → (렌더링 5) → **5/5**
  - **임계값은 낮추지 않았다.** 사후 조정은 검증이 아니라 사후 합리화다 (D12)

- **하네스 보강**
  - 빈 실행(사용량 한도, 인증 실패)을 실패와 분리한다. 도구 호출도 텍스트도 없으면
    `무효`로 표시하고 통과율 계산에서 제외한다. 한도가 계약 위반으로 둔갑하던 문제

- **1.7 비교 기준 베이스라인 확정**: 41/41 검사, 6/6 케이스
  (`2026-09-23T14-38-25`). 1.7은 Phase 1 최대 변경이므로 전후 비교가 필수다

- **열려 있는 발견**: 없음

- **다음**: ROADMAP 1.7 능력 선언과 의존성 디스패치
